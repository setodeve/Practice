...
class BankAccount{
    ...
    simulateTimePassage(days ,i){
        console.log(days);
        if(days != null){
            this.money = this.money * Math.pow((1 + i) ,days/365) ;
        }
        return this.money ; 
    }
...
function comeBackLaterPage(bankAccount){
    ...
    let nextBtn = container.querySelectorAll(".next-btn").item(0);

    let days = container.querySelector("#days-gone") ;

    nextBtn.addEventListener("click", function(){
        bankAccount.simulateTimePassage(days.value, 0.08);
        bankReturn(bankAccount);
